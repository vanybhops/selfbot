...
class Main(zeenode.Cog):
    ...
    async def ascii(self, ctx, *args):
        ...
        for arg in args:
            message = message + " " + arg
        text = pyfiglet.figlet_format(message)
    ...
    async def help(self, ctx, category=None):
        ...
        if category is None:
            ...
            embed.set_author(name="krtina krtina u | Prefix: " + str(bot.command_prefix),
                            icon_url="https://i.ytimg.com/vi/S7Anpm64mZ0/maxresdefault.jpg")
            embed.set_image(url="https://i.ytimg.com/vi/S7Anpm64mZ0/maxresdefault.jpg")
            ...
            embed.add_field(name="`\uD83D\uDCC3 - Text decoding`", value="Shows all **text decoding** commands.", inline=False)
            await ctx.send(embed=embed)
            ...
            await ctx.send(embed=embed)
        elif str(category).lower() == "textdecoding":
            embed = discord.Embed(color=0x0000, timestamp=ctx.message.created_at)
            embed.set_author(name="krtina krtina u | Prefix: " + str(bot.command_prefix),
                            icon_url="https://i.ytimg.com/vi/S7Anpm64mZ0/maxresdefault.jpg")
            embed.set_image(url="https://i.ytimg.com/vi/S7Anpm64mZ0/maxresdefault.jpg")
            embed.description = f"`\uD83D\uDCC3 - Text decoding Commands`\n`> decode_base64 <word/message>` - decodes text with Base64.\n`> decode_leet <word/message>` - decodes text with leet speak.\n`> decode_md5 <word/message>` - decodes text with MD5 hash.\n`> decode_sha1 <word/message>` - decodes text with Sha1.\n`> decode_sha224 <word/message>` - decodes text wish SHA224.\n`> decode_sha384 <word/message>` - decodes text with Sha384.\n`> decode_sha251 <word/message>` - decodes text with Sha512."
            await ctx.send(embed=embed)
